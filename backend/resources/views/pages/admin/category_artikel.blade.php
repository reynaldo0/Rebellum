...
@section('content')
            ...
            <tbody>
                @foreach ($categories as $category)
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200">
                        ...
                        <td class="px-6 py-4">
                            <button data-modal-target="modal-update-{{ $category->id }}"
                                data-modal-toggle="modal-update-{{ $category->id }}"
    ...
    <div id="modal-create" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative p-4 w-full max-w-md max-h-full">
            ...
            <div class="relative bg-white rounded-lg shadow-sm dark:bg-gray-700">
                ...
                <!-- Modal body -->
                <form method="post" action="{{ route('category-article.store') }}" class="p-4 md:p-5">
                    @csrf
    ...
    {{-- modal update --}}
    @foreach ($categories as $category)
        <div id="modal-update-{{ $category->id }}" tabindex="-1" aria-hidden="true"
            class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
            <div class="relative p-4 w-full max-w-md max-h-full">
                <!-- Modal content -->
                <div class="relative bg-white rounded-lg shadow-sm dark:bg-gray-700">
                    ...
                    <!-- Modal body -->
                    <form method="post" action="{{ route('category-article.update', $category) }}" class="p-4 md:p-5">
                        @csrf
                        @method('PUT')
                        <div class="grid gap-4 mb-4 grid-cols-2">
                            <div class="col-span-2">
                                ...
                                <input type="text" name="name" id="name"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                    placeholder="input kategori" value="{{ $category->name }}" required="">
                            </div>
                        </div>
                        <button type="submit"
                            class="text-white inline-flex w-full justify-center  items-center bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
