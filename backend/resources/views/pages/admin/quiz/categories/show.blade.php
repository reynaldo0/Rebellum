...
@section('content')
    <div class="max-w-4xl mx-auto bg-white shadow-lg rounded-lg p-6">
        <h2 class="text-2xl font-bold text-gray-800 mb-4">Detail Kategori: {{ $category->name }}</h2>

        <p class="text-gray-600 text-lg">Total Soal: {{ $category->quizzes->count() }}</p>
        <p class="text-gray-600 text-lg">Total Skor:
            <span class="{{ $category->totalScore() >= 100 ? 'text-green-600' : 'text-red-600' }}">
                {{ $category->totalScore() }} / 100
            </span>
        </p>

        <a href="{{ route('admin.quiz.create', ['category_id' => $category->id]) }}"
            class="mt-4 inline-block bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg transition">
            Tambah Soal
        </a>
                ...
                <tbody>
                    @foreach ($category->quizzes as $quiz)
                        <tr class="border-t hover:bg-gray-50">
                            ...
                            <td class="py-2 px-4 border font-bold text-blue-600">{{ $quiz->score }}</td>
                            <td class="py-2 px-4 border flex space-x-2">
                                ...
                                <form action="{{ route('admin.quiz.destroy', $quiz->id) }}" method="POST"
                                    ...
                                    @method('DELETE')
                                    <button type="submit"
                                        class="bg-red-600 hover:bg-red-700 text-white py-1 px-3 rounded-lg text-sm">
                                        Hapus
                                    </button>
                                </form>
                            </td>
        ...
        <a href="{{ route('admin.quiz.index') }}"
            class="mt-6 inline-block bg-gray-500 hover:bg-gray-600 text-white font-bold py-2 px-4 rounded-lg transition">
