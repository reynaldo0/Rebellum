...
@section('content')
    <div class="container mx-auto p-6 bg-white shadow-lg rounded-lg">
            ...
            <div id="modal-score"
                ...
                <div
                    ...
                    <p class="mt-4 text-gray-600">Skor Anda: <span
                            class="text-blue-600 font-semibold">{{ session('score') }}</span> dari {{ count($quizzes) }}</p>
        ...
        <form action="{{ route('scores.store') }}" method="POST" class="space-y-6">
            ...
            @endforeach
            <button type="submit"
                class="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg transition duration-300">Submit</button>
        </form>
    </div>
    ...
    @if (session('score'))
        <script>
            document.addEventListener("DOMContentLoaded", function() {
                ...
                // Tampilkan modal setelah submit dengan animasi fade-in dan scale-up
                setTimeout(() => {
