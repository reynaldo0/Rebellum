...
const FloatingButton = ({ setIsOpen }: ToUpProps) => {
  ...
  return (
    <div>
      <a
        ref={toUpRef}
        className="group/up flex fixed bottom-8 right-5 items-center justify-center gap-2 z-50 cursor-pointer rounded-full bg-yellow text-xl text-white opacity-100 transition duration-500 px-4 py-4 animate-bounce"
        onClick={() => setShowAdditionalButtons(!showAdditionalButtons)}>
        <IonIcon name="add-outline" className="text-2xl" />
      </a>

      {showAdditionalButtons && (
        <div className="fixed bottom-28 right-5 flex flex-col gap-2 z-50">
          <motion.a
            href="#konsultasi"
          ...
          <motion.a
            className="flex text-base gap-2 bg-[#445094] text-white px-4 py-2 rounded-full cursor-pointer"
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
